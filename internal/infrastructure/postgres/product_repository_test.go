...
func TestProductRepository_LiveIntegration(t *testing.T) {
	...
	// 1. Validation error on nil or invalid
	...
	// 2. Save new product with auto-generated UUID
	...
	// 4. FindByFingerprint
	byFp, err := repo.FindByFingerprint(ctx, p1.CurrentFingerprint)
	if err != nil {
		t.Fatalf("failed to find product by fingerprint: %v", err)
	}
	if byFp.ID != p1.ID {
		t.Errorf("expected ID %v, got %v", p1.ID, byFp.ID)
	}
	...
	// 6. Non-existent product queries
	...
	// 8. Transaction support via WithTx
