...
func TestRESTAdapter_RealWorldSuppliers(t *testing.T) {
	t.Run("ShopifyRESTProductCatalog", func(t *testing.T) {
		// Shopify Admin REST response with products array and Link header
		shopifyResp := `{
			"products": [
				{
					"id": 632910392,
		...
		records, nextCP, err := adapter.FetchRecords(context.Background(), "")
		if err != nil {
		...
		if records[0].ExternalProductID != "BURTON-SNOW-123" {
			t.Errorf("expected BURTON-SNOW-123, got %q", records[0].ExternalProductID)
		}
		if records[1].ExternalProductID != "BURTON-BIND-456" {
			t.Errorf("expected BURTON-BIND-456, got %q", records[1].ExternalProductID)
		}
		if nextCP != "https://example.com/products.json?page_info=next_page_token" {
			t.Errorf("expected next link checkpoint, got %q", nextCP)
		}
	})

	t.Run("AmazonSPAPICatalogItems", func(t *testing.T) {
		// Amazon SP-API items catalog response with pagination nextToken
		amazonResp := `{
		...
		if len(records) != 1 || records[0].ExternalProductID != "B07N4M94X4" {
			t.Fatalf("expected asin B07N4M94X4, got %v", records)
		}
		if nextCP != "cursor:amzn.nextToken.12345" {
			t.Errorf("expected cursor:amzn.nextToken.12345, got %q", nextCP)
		}
	})

	t.Run("WarehouseERPWithCompositeIdentity", func(t *testing.T) {
		// Warehouse feed with composite identity: warehouse_id + part_number
		erpResp := `{
			"code": 200,
			"payload": {
				"catalog": [
		...
		if records[0].ExternalProductID != "WH-DE:PART-100" {
			t.Errorf("expected WH-DE:PART-100, got %q", records[0].ExternalProductID)
		}
		if records[1].ExternalProductID != "WH-DE:PART-200" {
			t.Errorf("expected WH-DE:PART-200, got %q", records[1].ExternalProductID)
		}
		if nextCP != "cursor:scroll_abc_999" {
			t.Errorf("expected cursor:scroll_abc_999, got %q", nextCP)
		}
	})

	t.Run("SourceContractViolationOnMissingPath", func(t *testing.T) {
		// Supplier changed their API schema from "items" to "products"
	...
	t.Run("QuarantineThresholdEnforcement", func(t *testing.T) {
		// Supplier feed where 50% of items are missing identity field
		corruptedItemsResp := `{
			"items": [
				{"id": "VALID-1"},
				{"bad_key": "NO_ID"},
		...
		// Allow max 10% errors
		errorTracker := policy.NewErrorTracker(policy.PolicySkipMalformed, 0.10, nil)
		...
		_, _, err = adapter.FetchRecords(context.Background(), "")
		if err == nil {
			t.Fatal("expected quarantine threshold breach, got nil")
		}
...
func TestRESTAdapter_ErrorHandlingAndRateLimiting(t *testing.T) {
	...
	t.Run("ClientRateLimiterEnforcement", func(t *testing.T) {
		...
		start := time.Now()
		for range 3 {
			if _, _, err := adapter.FetchRecords(ctx, ""); err != nil {
